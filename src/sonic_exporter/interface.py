...
metric_interface_transceiver_info = GaugeMetricFamily(
...
def export_interface_counters():
    ...
    for ifname in maps:
        ...
        metric_interface_info.add_metric(
            [
        ...
        metric_interface_speed.add_metric(
            [get_additional_info(ifname)],
        ...
        for size, key in zip(
            ...
            metric_interface_received_ethernet_packets.add_metric(
                [get_additional_info(ifname), str(size)],
                floatify(_decode(getFromDB(sonic_db.COUNTERS_DB, counter_key, key))),
            )
        # Ethernet TX
        for size, key in zip(
            (64, 127, 255, 511, 1023, 1518, 2047, 4095, 9216, 16383),
            ...
            metric_interface_transmitted_ethernet_packets.add_metric(
                [get_additional_info(ifname), str(size)],
                floatify(_decode(getFromDB(sonic_db.COUNTERS_DB, counter_key, key))),
            )
        # RX
        metric_interface_received_bytes.add_metric(
            [get_additional_info(ifname)],
            floatify(
                getFromDB(
        ...
        metric_interface_received_packets.add_metric(
        ...
        # RX Errors
        metric_interface_receive_error_input_packets.add_metric(
        ...
        # TX
        metric_interface_transmitted_bytes.add_metric(
            [get_additional_info(ifname)],
            floatify(
                getFromDB(
                    sonic_db.COUNTERS_DB,
        ...
        metric_interface_transmitted_packets.add_metric(
            [get_additional_info(ifname), "unicast"],
            floatify(
                getFromDB(
                    sonic_db.COUNTERS_DB,
        ...
        # TX Errors
        metric_interface_transmit_error_output_packets.add_metric(
            [get_additional_info(ifname), "error"],
            floatify(
                getFromDB(
                    sonic_db.COUNTERS_DB,
        ...
        try:
            ...
            metric_interface_operational_status.add_metric(
                [get_additional_info(ifname)], boolify(is_operational)
            )
            metric_interface_admin_status.add_metric(
                [get_additional_info(ifname)], boolify(is_admin)
            )
            metric_interface_last_flapped_seconds.add_metric(
...
def export_interface_queue_counters():
    ...
    for ifname in maps:
        ...
        metric_interface_queue_processed_packets.add_metric(
            [get_additional_info(ifname), queue, queue_type], floatify(packets)
        )
        metric_interface_queue_processed_bytes.add_metric(
            [get_additional_info(ifname), queue, queue_type], floatify(bytes)
        )


def export_interface_optic_data():
    ...
    for key in keys:
        ...
        for measure in transceiver_sensor_data:
            ...
            try:
                ...
                match measure_dec:
                    case "voltage":
                        metric_interface_optic_volts.add_metric(
                    ...
                    case "temperature":
                        metric_interface_optic_celsius.add_metric(
                            [ifname, get_additional_info(ifname)],
                            floatify(value),
                        )
                    case _:
                        if match := rx_power_regex.fullmatch(measure_dec):
                            ...
                            metric_interface_receive_optic_power_dbm.add_metric(
                                [
                                    ifname,
                                    get_additional_info(ifname),
                                    optic_unit,
                            ...
                            optic_unit = match.group(1)
                            metric_interface_transmit_optic_power_dbm.add_metric(
                                [
                                    ifname,
                                    get_additional_info(ifname),
                                    optic_unit,
                            ...
                            metric_interface_transmit_optic_bias_amperes.add_metric(
                                [
                                    ifname,
                                    get_additional_info(ifname),
                                    optic_unit,
                ...
                pass
        metric_transceiver_threshold_info.add_metric(
            [
                ifname,
                get_additional_info(ifname),
                vcchighalarm,
...
def export_interface_cable_data():
    ...
    for key in keys:
        ...
        part_number = _decode(getFromDB(sonic_db.STATE_DB, key, "vendor_part_number"))
        revision = _decode(getFromDB(sonic_db.STATE_DB, key, "vendor_revision"))
        form_factor = _decode(getFromDB(sonic_db.STATE_DB, key, "form_factor")).lower()
        ...
        try:
            ...
            metric_interface_cable_length_meters.add_metric(
        ...
        metric_interface_transceiver_info.add_metric(
            [
                get_additional_info(ifname),
                serial,
                part_number,
