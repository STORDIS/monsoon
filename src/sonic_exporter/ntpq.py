...
class NTPQ:
    def run_command(self, command: list, db_version: ConfigDBVersion, vrf: Optional[str]=None):
        ...
        if self.vrf:
            if db_version < ConfigDBVersion("version_4_0_0"):
                new_command = ["cgexec", "-g", f"l3mdev:{vrf.strip()}", "ntpq"] + command
            else:
    ...
    def get_associations(self, db_version: ConfigDBVersion = ConfigDBVersion("version_3_4_1"), vrf: Optional[str] = None):
